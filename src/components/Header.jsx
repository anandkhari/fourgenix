...
const Header = () => {
  ...
  // Desktop-only BG logic:
  // Home => transparent
  // Others => light blue (#DCDEFF)
  const desktopBg =
    pathname === "/" ? "md:bg-transparent" : "md:bg-[#E1E3FF]";

  return (
    <header
      className={`absolute left-0 top-0 z-100 w-full py-2 px-4 
        bg-[#DCDEFF]   /* Mobile background always */
        ${desktopBg}   /* Desktop logic */
      `}
    >
      ...
      {/* Mobile Dropdown */}
      {isOpen && (
        <div className="md:hidden fixed top-[88px] left-0 w-full bg-white shadow-md rounded-b-lg z-50">
          <ul className="flex flex-col">
            {menuItems.map((item, idx) => (
              <li key={item.href}>
                <a
                  href={item.href}
                  className="body text-lg font-medium text-gray-700 hover:text-black hover:bg-white/10 py-4 px-6 flex w-full"
